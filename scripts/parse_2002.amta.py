...
with open(FILE_NAME+".tsv", 'w') as f:
    ...
    for p in page_lines:
        ...
        try:
             title = re.split(":", p.get_text())[1:]#Remove the first text before ":"
             title = ":".join(title)
             
             title = re.split("; pp", title)[0]
             title = re.split("pp.", title)[0].strip().strip(';')
             title = unspace(title)
        except:
            continue
...
print('=*'*10)
print('Your TSV file is written. Please check for ./'+FILE_NAME+'.tsv')
print('Make sure to remove inapropriate lines and the "raw_text" column in it before using it')