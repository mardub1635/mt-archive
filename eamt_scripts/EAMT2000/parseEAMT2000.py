...
# save webpage
FILE_NAME = "EAMT2000"
URL = 'http://www.mt-archive.info/00/EAMT-2000-TOC.htm'
...
def getTitle(p):
    ...
    if p.find_all(['a']):
        titles=p.find_all(['a'])
        t=titles[0].get_text()#.decompose()#.contents[0]
        t=t.replace("--","").replace("\n"," ").strip()
        return t
    return None
...
def getAuthors(p):
    if p.find_all(['i']):
        i = p.find_all(['i'])
        i=i[0].get_text().replace("\"","").replace("\n","").strip()
        authors=HumanName(i)
        authors=authors.last+", "+authors.first
        return(authors)
    return None
    #print(Author)
    return Author
