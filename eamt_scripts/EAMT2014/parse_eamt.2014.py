...
def has_arial(p):
    return p.find(style="font-family:Arial")
...
with open(FILE_NAME+".tsv", 'w') as f:
    ...
    writer.writerow(["Title", "Authors", "Pdf", "Presentation","Volume_Name","Raw_webtext"])
    presentation=None
    track=None
    for p in pagelines[5:]:
        ...
        title = get_title(p)
        if has_arial(p):
            track = has_arial(p).get_text().replace("\n","")
        ...
        if pdf:
            pdf = urljoin(URL, pdf)
        authors = namify(p.getText().split(":")[0])
